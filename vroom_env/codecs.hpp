...
struct header {
    ...
    static uint8_t const* read(uint8_t const* in, uint32_t* n,
                               uint32_t* universe) {
...
struct optpfor {
                       ...
                       uint8_t const* b = nullptr)  // if non-null forces b
    {
        ...
        optpfor_codec.encodeArray(in, n,
        ...
        uint32_t decodeBlock(uint8_t const*& src, uint32_t* dst) const {
            ...
            if (readSize > 4) {
                const __m128i result2 =
                    _mm_shuffle_epi8(data, vecmask[desc][1]);
...
struct pef {
    ...
    typedef positive_sequence<partitioned_sequence<strict_sequence>>
        freqs_sequence_type;

    static void encode(uint32_t const* in, uint32_t universe, uint32_t n,
                       succinct::bit_vector_builder& bvb, bool freqs) {
    ...
    static void decode(succinct::bit_vector const& bv, uint32_t* out,
        ...
        if (freqs) {
            auto e = freqs_sequence_type::enumerator(bv, offset, universe, n,
                                                     params);
            ...
            auto e =
                docs_sequence_type::enumerator(bv, offset, universe, n, params);
...
#define CODECS                                                       \
    (interpolative)(optpfor)(varintg8iu)(qmx)(vbyte)(u32)(simple16)( \
        streamvbyte)(maskedvbyte)(varintgb)
}  // namespace ds2i
