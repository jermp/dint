    ...
    struct dict_freq_index
    {
            ...
            {
                {
                    ...
                    build_or_load_dict(m_docs_large_dict_builder, m_docs_small_dict_builder,
...

                {
                    ...
                    build_or_load_dict(m_freqs_large_dict_builder, m_freqs_small_dict_builder,
                                       prefix_name, data_type::freqs);
                    logger() << "DONE" << std::endl;
                }

                m_docs_large_dict_builder.prepare_for_encoding();
                m_freqs_large_dict_builder.prepare_for_encoding();
                m_docs_small_dict_builder.prepare_for_encoding();
                m_freqs_small_dict_builder.prepare_for_encoding();
            }
        ...
        private:
            ...
            struct list_adder : semiasync_queue::job {
                ...
                {
                    sequence_type::write(b.m_docs_large_dict_builder, b.m_freqs_large_dict_builder,
                                         b.m_docs_small_dict_builder, b.m_freqs_small_dict_builder,
            ...
            void build_or_load_dict(typename large_dictionary_type::builder& large_dict_builder,
                                    ...
                                    std::string prefix_name, data_type dt)
            {
                ...
                std::string large_dictionary_filename = "./dict."
                                            + p.filename().string() + "."
                                            + ld_type::type() + "."
                                            + dictionary_builder::type() + ".large";
                using sd_type = typename small_dictionary_type::builder;
                std::string small_dictionary_filename = "./dict."
                                            + p.filename().string() + "."
                                            + sd_type::type() + "."
                                            + dictionary_builder::type() + ".small";

                if (boost::filesystem::exists(large_dictionary_filename) and
                    boost::filesystem::exists(small_dictionary_filename))
                {
                    large_dict_builder.load_from_file(large_dictionary_filename);
                    small_dict_builder.load_from_file(small_dictionary_filename);
                } else {
                    ...
                    dictionary_builder::build(large_dict_builder,
                                              small_dict_builder,
                                              statistics, dt);

                    if (not large_dict_builder.try_store_to_file(large_dictionary_filename)) {
                        logger() << "cannot write dictionary to file";
                    }

                    if (not small_dict_builder.try_store_to_file(small_dictionary_filename)) {
                        logger() << "cannot write dictionary to file";
                    }
                }
        ...
        document_enumerator operator[](size_t i) const
        {
            ...
            return document_enumerator(&m_docs_large_dict, &m_freqs_large_dict,
