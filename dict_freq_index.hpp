    ...
    struct dict_freq_index {
        ...
        struct builder {
            ...
            void build_model(InputCollection const& input, std::string const& prefix_name)
            {
                    ...
                    for (auto const& plist: input)
                    {
                        ...
                        if (n > MIN_SIZE)
                        {
                            ...
                            while (docs_begin != docs_end) {
                                gaps.push_back(*docs_begin - prev - 1);
                                prev = *docs_begin;
                                ++docs_begin;
                            }
                            assert(gaps.size() == n);
                ...
                DictBuilder::build2(m_docs_dict_builder, total_integers, "./" + prefix_name + ".docs");
                ...
                DictBuilder::build2(m_freqs_dict_builder, total_integers, "./" + prefix_name + ".freqs");
