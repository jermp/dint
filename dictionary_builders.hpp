...
namespace ds2i {
    ...
    struct dint_dictionary_builder
    {
        ...
        static void build(dictionary::builder& builder, uint64_t total_integers,
                          std::string prefix_name, double eps = 0.0001)
        {
            builder.init(num_entries, entry_width);
            ...
            std::vector<std::vector<entry_type2>> candidates(MAX_FRACTAL_STEPS,
                                                             std::vector<entry_type2>());
