...
namespace ds2i {
    ...
    struct dint {
        ...
        static
        uint8_t const* decode(Dictionary const& dict,
                              uint8_t const* in,
                              uint32_t* out,
                              size_t n
                              // , dint_statistics& stats
                             )
        {
            ...
            for (size_t i = 0; i != n; ++ptr) {
                ...
                if (DS2I_LIKELY(index > EXCEPTIONS - 1))
                {
                    ...
                    //     stats.ints_distr[4] += 8;
                    //     stats.codewords_distr[4] += 1;
                    // } else if (decoded_ints == 16 and index > Dictionary::reserved - 1) {
                    //     stats.ints_distr[5] += 16;
                    //     stats.codewords_distr[5] += 1;
                    // } else if (decoded_ints >= 16) {
                    //     stats.ints_distr[0] += decoded_ints;
    ...
    struct greedy_dint {
        ...
        {
            return dint::decode(dict, in, out, n
                    // , stats
                );
        }
    ...
    struct opt_dint
    {
        ...
        // single-dictionary
        template<typename SingleDictionary>
        static
        uint8_t const* decode(SingleDictionary const& single_dict,
                              uint8_t const* in,
                              uint32_t* out,
                              uint32_t /*universe*/,
                              size_t n
        ...
        {
            return dint::decode(single_dict, in, out, n
                    // , stats
                );
        }
        ...
        //             encode(builder, s, begin, size, encoded[s + constants::num_selectors], 8);
        ...
        // static
        // uint8_t const* decode(MultiDictionary const& multi_dict,
        //                       uint8_t const* in,
        //                       uint32_t* out,
        //                       uint32_t /*universe*/,
        //                       size_t n
