...
enum class dint_data_type : char
{
     docs='d',
     freqs='f',
};
...
namespace ds2i {
    ...
    template<class dict_strategy,class t_dict_builder>
    void build_or_load_dict(t_dict_builder& dict,std::string prefix_name,dint_data_type data_type)
    {
        std::string file_name = prefix_name + ".docs";
        if(data_type == dint_data_type::freqs) file_name = prefix_name + ".freqs";
        std::string dict_file = file_name + "." + dict_strategy::type();
        if( boost::filesystem::exists(dict_file) ) {
            dict.load_from_file(dict_file);
        } else {
            using block_stats_type = typename dict_strategy::block_stats_type;
            auto block_stats = block_stats_type::create_or_load(prefix_name,data_type);
            dict_strategy::build(dict,block_stats);
            if(! dict.try_store_to_file(dict_file) ) {
                DS2I_LOG << "cannot store dictionary to file";
            }
        }
    }

    template<typename t_block_stats_type,
             uint32_t num_entries = 65536,
             uint32_t entry_width = 16
             >
    struct dint_dict_strategy_DSF
    {
        using dict_type = dictionary<entry_width>;
        using builder_type = typename dict_type::builder;
        using block_stats_type = t_block_stats_type;
        ...
        static void build(builder_type& builder,block_stat_type& block_stats)
        {
            // (1) init dictionary
            DS2I_LOG << "(1) init dictionary";
            builder.init(num_entries,type());
    ...
    {
        using builder_type = typename dictionary<entry_width>::builder;
