...
namespace ds2i {
    ...
    struct multi_dictionary {
        ...
        struct builder {
            ...
            static const uint32_t num_dictionaries = multi_dictionary::num_dictionaries;
            ...
            void write(std::ofstream& dictionary_file) const {
                ...
                dictionary_file.write(reinterpret_cast<char const*>(&start_offsets_size), sizeof(uint32_t));
                ...
                dictionary_file.write(reinterpret_cast<char const*>(m_start_offsets.data()), start_offsets_size * sizeof(uint32_t));
            ...
            size_t load(std::ifstream& dictionary_file) {
                ...
                dictionary_file.read(reinterpret_cast<char*>(&start_offsets_size), sizeof(uint32_t));
            ...
            bool append(uint32_t const* entry, uint32_t entry_size, uint32_t dictionary_id)
            {
            ...
            void prepare_for_encoding()
            {
                ...
                for (uint32_t dictionary_id = 0;
                              dictionary_id != num_dictionaries;
                            ++dictionary_id)
                {
            ...
            uint32_t lookup(uint32_t dictionary_id,
                            uint32_t const* begin,
                            uint32_t entry_size,
                            uint32_t log2_num_entries) const
            {
                assert(log2_num_entries == 8 or
                       log2_num_entries == 16);
                assert(dictionary_id < num_dictionaries);

                uint64_t hash = hash_bytes64(begin, entry_size);
                auto const& map = m_maps[dictionary_id + (log2_num_entries == 8) * num_dictionaries];
        ...
        {
            visit
                (m_start_offsets, "m_start_offsets")
                (m_offsets, "m_offsets")
                (m_table, "m_table")
                ;
        }
