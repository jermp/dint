/*
        qmx.hpp (modified by Matthias Petri
                 and Giulio Ermanno Pibiri)
...
struct codec {
    ...
    codec(uint32_t n) {
        length_buffer.resize(n + constants::WASTAGE);
        full_length_buffer.resize(n + constants::WASTAGE);
        source_integers = n;
    }
    ...
    size_t encode(void* dest_as_void, const uint32_t* src)
    {
        ...
        cur_len = len_buf;
        for (cur = (uint32_t*) src; cur < src + source_integers; cur++)
        ...
        */
        for (uint8_t* cl = len_buf; cl < len_buf + source_integers + 4; cl += 4) {
        ...
        */
        cur_len = len_buf;
        while (cur_len < len_buf + source_integers) {
            /*
            ...
            */
            if (source_integers - (cur_len - len_buf) < 4) {
                largest = 0;
                ...
                if (largest <= 8)
                    ...
                    for (block = 0; block < 8; block++)
                        *(cur_len + block) = 32;
            } else if (source_integers - (cur_len - len_buf) < 8) {
                largest = 0;
                for (block = 0; block < 8; block++)
                    largest = util::maximum((uint8_t)largest, *(cur_len + block));
                if (largest <= 8)
                    ...
                    for (block = 0; block < 8; block++)
                        *(cur_len + block) = 16;
            } else if (source_integers - (cur_len - len_buf) < 16) {
                largest = 0;
        ...
        keys = len_buf;
        for (cur = (uint32_t*)src + 1; cur < src + source_integers; cur++) {
