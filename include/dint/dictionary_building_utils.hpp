...
namespace ds2i {
    ...
    struct target_t {
        ...
        bool operator<(const target_t& other) const {
            if(entry.size() == other.entry.size()) {
                return std::lexicographical_compare(entry.begin(),entry.end(),other.entry.begin(),other.entry.end());
            }
            return entry.size() < other.entry.size();
        }

        bool operator==(const target_t& other) const {
            if(entry.size() != other.entry.size() ) return false;
            return std::equal(entry.begin(),entry.end(),other.entry.begin(),other.entry.end());
        }

        bool operator!=(const target_t& other) const {
            if(entry.size() != other.entry.size() ) return true;
            return std::equal(entry.begin(),entry.end(),other.entry.begin(),other.entry.end());
        }
    ...
    size_t compute_overlap(const std::vector<uint32_t>& A,const std::vector<uint32_t>& B) {
    ...
    std::priority_queue<overlap_t> compute_all_overlaps(std::vector<target_t>& entries) {
    ...
    void perform_greedy_prefix_suffix_overlap(std::vector<target_t>& entries) {
        ...
        while(!overlaps.empty()) {
            auto cur = overlaps.top(); overlaps.pop();
            if(!entries[cur.left].valid || !entries[cur.right].valid) {
                continue;
            }

            // std::cout << merges << " overlaps = " << overlaps.size() << " " << cur.overlap << std::endl;

            // (a) merge the nodes
            overlap_sum += cur.overlap;
            auto& node_left = entries[cur.left];
            auto copy_itr = entries[cur.right].entry.begin() + cur.overlap;
            std::copy(copy_itr,entries[cur.right].entry.end(),std::back_inserter(node_left.entry));
            ...
            for(size_t j=0;j<entries.size()-1;j++) {
                ...
                auto overlap_left = compute_overlap(entries[j].entry,entries.back().entry);
                auto overlap_right = compute_overlap(entries.back().entry,entries[j].entry);
                if(overlap_left > 1) {
        ...
        for(size_t i=0;i<entries.size();i++) {
            if(entries[i].valid == false) continue;
            auto last_sym = entries[i].entry.back();
            for(size_t j=0;j<entries.size();j++) {
                if(i != j) {
                    if(entries[j].valid == false) continue;
                    auto first_sym = entries[j].entry.front();
                    if(first_sym == last_sym) {
                        entries[j].valid = false;
                        total_merges_performed++;
                        std::copy(entries[j].entry.begin()+1,entries[j].entry.end(),std::back_inserter(entries[i].entry));
                        break;
                    }
                }
            }
